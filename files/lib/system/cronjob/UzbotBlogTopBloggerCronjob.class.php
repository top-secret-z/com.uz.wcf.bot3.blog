...
class UzbotBlogTopBloggerCronjob extends AbstractCronjob
{
    ...
    public function execute(Cronjob $cronjob)
    {
        ...
        $bots = UzbotValidBotCacheBuilder::getInstance()->getData(['typeDes' => 'blog_topBlogger']);
        if (empty($bots)) {
            return;
        }
        ...
        foreach ($bots as $bot) {
            ...
            if (!$bot->testMode) {
                if ($bot->topBloggerNext > TIME_NOW) {
                    continue;
                }
            }
            ...
            if (\count($userIDs)) {
                $conditionBuilder->add('entry.userID IN (?)', [$userIDs]);
            } else {
                $conditionBuilder->add('entry.userID > ?', [0]);
            }
